...
void OilCommandAsmCompiler::InitializeStackOperations() {
  // PushNull
  std::vector<AssemblyInstruction> push_null_asm = {{AsmCommand::MOV, {Register::RAX, static_cast<int64_t>(0)}},
                                                    {AsmCommand::PUSH, {Register::RAX}}};
  AddStandardAssembly("PushNull", std::move(push_null_asm));

  // Pop
  std::vector<AssemblyInstruction> pop_asm = {{AsmCommand::POP, {Register::RAX}}};
  AddStandardAssembly("Pop", std::move(pop_asm));

  // Dup
  std::vector<AssemblyInstruction> dup_asm = {{AsmCommand::MOV, {Register::RAX, create_memory_addr(Register::RSP)}},
                                              {AsmCommand::PUSH, {Register::RAX}}};
  AddStandardAssembly("Dup", std::move(dup_asm));

  // Swap
  std::vector<AssemblyInstruction> swap_asm = {{AsmCommand::POP, {Register::RBX}},
  ...
  // NullCoalesce: a ?? b
  std::vector<AssemblyInstruction> null_coalesce_asm = {{AsmCommand::POP, {Register::RBX}},
                                                        {AsmCommand::POP, {Register::RAX}},
                                                        {AsmCommand::TEST, {Register::RAX, Register::RAX}},
